...
function getGitDirPath(repoRootPath: string): string | null {
  ...
  try {
    ...
    if (stat.isFile()) {
      ...
      return path.isAbsolute(gitdirRaw)
        ? gitdirRaw
        : path.resolve(repoRootPath, gitdirRaw);
    }
  } catch {
    return null;
  }
...
export class CourseRepoResolver {
  ...
  async resolveCourseContextFromRepo(): Promise<CourseRepoContext | null> {
    ...
    for (const course of courses) {
      ...
      if (
        remoteText.toLowerCase().includes(course.display_name.toLowerCase())
      ) {
        score += 1;
      }
