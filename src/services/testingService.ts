...
import type {
  AutoGraderDetails,
  AutoGraderDetailsData,
  TestCase,
  Autocheck,
} from '../interfaces/AutoGraderDetails';
...
export class TestingService {
   ...
   */
  runGradeableWithResult(
    term: string,
    courseId: string,
    gradeableId: string,
    label: string,
    result: AutoGraderDetails
  ): void {
  ...
  private formatAutocheckOutput(
    autochecks: Autocheck[] | undefined,
    getValue: (ac: Autocheck) => string
  ): string {
  ...
  private formatAutocheckMessages(autochecks: Autocheck[] | undefined): string {
    ...
    const parts = autochecks.map(ac => {
      const msgLines = (ac.messages ?? []).map(
        m => `  • ${m.message}${m.type ? ` (${m.type})` : ''}`
      );
    ...
    item.children.forEach(child => {
      ...
      if (tc) {
        ...
        if (passed) {
          ...
          msg.expectedOutput = this.formatAutocheckOutput(
            tc.autochecks,
            ac => ac.expected
          );
          msg.actualOutput = this.formatAutocheckOutput(
            tc.autochecks,
            ac => ac.actual
          );
